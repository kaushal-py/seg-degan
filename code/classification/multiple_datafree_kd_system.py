...
class MultipleDatafreeKDSystem:
    def __init__(self, config, hparams):
        ...
        self.Gs = []
        for checkpoint in self.hparams.generator_checkpoints:
            G = dcgan_model.Generator(ngpu=1, nz=self.hparams.nz)
            generator_checkpoint = torch.load(checkpoint)
            G.load_state_dict(generator_checkpoint['g_state_dict'])
    ...
    def train_step(self, batch_idx):
        ...
        with torch.no_grad():
            ...
            selected_G = np.random.randint(0, len(self.Gs))
            data = self.Gs[selected_G](noise).detach()
